...
class ForwardFusedMoE_CutlassSwiGLU_BF16(FusedOperation):
    ...
    def grouped_swiglu_kernel(cls) -> Callable:
        """Probe the QuACK gemm_gated backend (raises ImportError if QuACK is missing).

        The fused MoE is implemented entirely with QuACK (gemm_gated up-proj+SwiGLU,
        gemm/gemm_dgated backward); ``is_supported`` probes it exactly like the reference
        probes its cuDNN-FE imports.
        """
        try:
            from quack.gemm_interface import gemm_gated  # pylint: disable=import-outside-toplevel

            return gemm_gated
        except ImportError as e:
            raise ImportError("quack.gemm_interface.gemm_gated") from e
    ...
    def fuser_forward(
        ...
        *,
        ...
        w1 = self._get_fc1_weight_for_quack(fc1_op, num_groups, fc1_weight_shape, dtype)
        ...
        # All-QuACK fused MoE up-proj + SwiGLU + (interleaved) pre-activation h, in ONE gemm_gated.
        # concat_layout=("B",) consumes the SAME plain [G*2I,d] weights (no re-interleave -> Muon /
        # checkpoint-safe). Ragged tokens via cu_seqlens (no 256-pad). gemm_gated has no per-token prob
        # param, so the router-gate multiply is applied post-hoc. h is always stored when grad is
        ...
        B_gated = (
            ...
            else w1.view(num_groups, two_i, d).permute(0, 2, 1)
        )
        ...
        # FC2 down-proj via QuACK gemm: Y[M,H] = A[M,I] @ W2[G*H,I]^T per expert.  B = W2^T = [E,I,H].
        w2_2d = self._get_fc2_weight_for_quack(fc2_op, num_groups, fc2_weight_shape, dtype)
        B_down = (
            w2_2d
            if isinstance(w2_2d, list)
            else w2_2d.view(num_groups, H, I).transpose(1, 2)
        )  # [E,I,H] = W2^T per expert, or list of [I,H]
    ...
    def _get_fc1_weight_for_quack(
        ...
        # Current path: stack into one dense grouped-B tensor.
    ...
    def _get_fc2_weight_for_quack(
        ...
        See ``_get_fc1_weight_for_quack`` for macro semantics.
        """
