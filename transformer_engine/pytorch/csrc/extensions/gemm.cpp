...
std::optional<std::vector<at::Tensor>> te_general_grouped_gemm(
  ...
  for (size_t i = 0; i < A.size(); i++) {
    ...
    printf("A Shape\n(");
    for (size_t i = 0; i < te_A.shape().ndim; ++i) {
      printf("%d, \t", te_A.shape().data[i]);
    }
    printf(")\n");

    printf("B Shape\n(");
    for (size_t i = 0; i < te_B.shape().ndim; ++i) {
      printf("%d, \t", te_B.shape().data[i]);
    }
    printf(")\n");

    printf("=== D shape with transa: %d, transb: %d\n(", transa, transb);
    //*/
...
std::optional<std::vector<at::Tensor>> cublas_general_grouped_gemm(
    std::vector<py::handle> A, bool transa, std::vector<py::handle> B, bool transb,
  ...
  // For now, we only have multi-stream cublas backend.
  nvte_cublas_grouped_gemm(te_A_vector.data(), te_B_vector.data(), te_D_vector.data(),
